import type {ColorInfoType} from "~/types/common";

const colorToString = (color: ColorInfoType, short = false) => {
  if (!color) return ''
  const {
    standard,
    white,
  ...
  const isEbony = () => {
    if (ebony !== 'no') {
      if (beige === 'no' || ebony === 'carrier') return ebonyName() + ' '
      else return ''
    } else return ''
  }
  ...
  const isViolet = (POSITION: number) => {
  ...
  const isSapphire = (POSITION: number) => {
  ...
  const isAngora = (POSITION: number) => {
    if (angora !== 'no') {
      if (POSITION === 1) {
        switch (angora) {
          case 'yes':
  ...
  const isCalifornia = (POSITION: number) => {
    if (california !== 'no') {
      if (POSITION === 1) {
        switch (california) {
          case 'yes':
  ...
  const isGerman = (POSITION: number) => {
